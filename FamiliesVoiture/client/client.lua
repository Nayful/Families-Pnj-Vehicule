...
familiesMenu = NativeUI.CreateMenu("Garage Families", "Liste des véhicules Families", nil, nil, "shopui_title_ie_modgarage", "shopui_title_ie_modgarage")
_menuPool:Add(familiesMenu)
...
spawnCar = function(car)
    ...
    local vehicle = CreateVehicle(car, -153.9, -1654.88, 32.75, 134.5, true, false)
    SetEntityAsMissionEntity(vehicle, true, true)
    SetVehicleNumberPlateText(vehicle, "Families")
...
function AddFamiliesMenu(menu)
    ...
    voituremenu.SubMenu.OnItemSelect = function(menu, item)
        if item == sultan then 
            ESX.ShowAdvancedNotification("Families", "Actions Effectué", "Voici Ton Véhicule Man ! Fais gaffe à toi !", "CHAR_CARSITE", 1)
            Citizen.Wait(1)
            spawnCar("sultan")
...
AddFamiliesMenu(familiesMenu)
_menuPool:RefreshIndex()



local families = {
    {x = -149.73, y = -1663.5, z = 32.82},
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(families) do

            local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
            local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, families[k].x, families[k].y, families[k].z)

            if dist <= 1.2 then
             if PlayerData.job2 ~= nil and PlayerData.job2.name == 'families' then ---------- Le grade qui à acces au menu
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_TALK~ pour intéragir avec ~b~CJ")
                if IsControlJustPressed(1,51) then 
                    latestPoint = families[k].type 
                    familiesMenu:Visible(not familiesMenu:Visible())
                end
            end
            end
        end
...
local v1 = vector3(-149.45, -1664.09, 33.80)   -- Le nom au dessus du PNJ
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVMALE", "g_m_y_famdnf_01", -149.45, -1664.09, 31.83, 29.29, false, true) --Emplacement du PEDS
